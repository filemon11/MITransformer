...
means <- aggregate(cbind(RT, surprisal.s, head_distance, first_dependent_distance, left_dependents_distance_sum, left_dependents_count) ~ item + zone,
...
cor.test(means$RT, -means$head_distance, method="pearson")
cor.test(means$RT, -means$first_dependent_distance, method="pearson")
cor.test(means$RT, means$left_dependents_distance_sum, method="pearson")
cor.test(means$RT, means$left_dependents_count, method="pearson")
...
for (x in 0:(num.models-1)) {
  means <- aggregate(cbind(RT, surprisal.s) ~ item + zone,
                     data = get(paste("d1", x, sep = ".")), FUN = mean, na.rm = TRUE)

  c <- cor.test(means$RT, means$surprisal.s, method="pearson")
  ...
  plot(means$surprisal.s, means$RT, xlim=c(0,60))+
    abline(lm(means$RT ~ means$surprisal.s))
  dev.off()
}
...
# test head_distance RT correlation
# cor.test(means$RT, means$head_distance, method="pearson")
# pdf(file="results/plot_head_distance.pdf")
# plot(means$head_distance, means$RT, xlim=c(0,60))+
#  abline(lm(means$RT ~ means$head_distance))
# dev.off()
...
for (x in 0:(num.models-1)) {
  # ---------------------- Step 1 ----------------------
  s0 <- lmer(RT ~ frequency.s + length.s + 
                 (1|WorkerId) + (1|item),
  ...
  s1 <- lmer(RT ~ frequency.s + length.s + surprisal.s +
               (1|WorkerId) + (1|item),
             data=na.omit(get(paste("d1", x, sep = "."))),
             REML=FALSE)
...
# 
#     # ---------------------- Step 1 ----------------------
#     m0 <- lmer(RT.y ~ frequency.s + length.s + surprisal.s +
#                  (1|WorkerId) + (1|item),
...
# 
#     # ---------------------- Step 2 ----------------------
#     m1 = lmer(RT.y ~ frequency.y + length.y + surprisal.s.y + surprisal.s.x +
#                   (1|WorkerId) + (1|item),
#                 data=na.omit(newdata),
#                 REML=FALSE)
...
#   }
# }
# prst$prs1 <- sqrt(prst$prs1 / num.models)
# 
# print("pr mean")
# print(pr)
# print("pr std")
...
for (subset_name in names(subset_conditions)) {
  ...
  for (i in 0:num.models) {
    ...
    if (i == 0) {
      ...
      print(nrow(aggregate(RT ~ item + zone,
    ...
    if (nrow(data_subset) > 0) {
      ...
      # Fit models
      model_without_surprisal <- lmer(RT ~ frequency.s + length.s + (1 | WorkerId) + (1 | item), 
                                 data = data_subset, REML = FALSE)
      
      model_with_surprisal <- lmer(RT ~ frequency.s + length.s + surprisal.s + (1 | WorkerId) + (1 | item), 
                              data = data_subset, REML = FALSE)
...
# Define subset conditions
subset_conditions <- list(
  "first_dependent_correct == 0"  = function(d) d$first_dependent_correct == 0,
  "first_dependent_correct == 1" = function(d) d$first_dependent_correct == 1
)
...
for (subset_name in names(subset_conditions)) {
  ...
  for (i in 0:num.models) {
    ...
    elemnum <- nrow(aggregate(RT ~ item + zone,
                              data = data_subset, FUN = mean, na.rm = TRUE))
    if (i == 0) {
      print(paste("Number of elements in ", subset_name))
      print(elemnum)
    ...
    if (nrow(data_subset) > 0) {
      ...
      model_with_surprisal <- lmer(RT ~ frequency.s + length.s + surprisal.s + (1 | WorkerId) + (1 | item), 
                              data = data_subset, REML = FALSE)
      a <- anova(model_without_surprisal, model_with_surprisal)  # Significant?
      delta_logLik <- as.numeric((a$logLik[2]-a$logLik[1])/elemnum)
...
# Print results
print("Refittet models for first_dependent_correct bins")
print(results)
...
# Compute mean RT for each (first_dependent_distance, first_dependent_distancedeprels) pair
rt_means <- data %>%
  group_by(first_dependent_distance, first_dependent_distancedeprels) %>%
  summarise(Mean_RT = mean(RT, na.rm = TRUE), SD_RT = sd(RT, na.rm = TRUE), .groups = "drop")

# Print results
print("Mean RT aggregated per raw first_dependent_distance value and first_dependent_distancedeprels")
print(rt_means)

# Plot results: separate line for each first_dependent_distancedeprels
ggplot(rt_means, aes(x = first_dependent_distance, y = Mean_RT, color = as.factor(first_dependent_distancedeprels), group = first_dependent_distancedeprels)) +
  geom_point(size = 2, alpha = 0.6) +  # Scatter plot
  geom_line(alpha = 0.8) +  # Line plot
  geom_errorbar(aes(ymin = Mean_RT - SD_RT, ymax = Mean_RT + SD_RT), width = 0.2, alpha = 0.5) +  # Error bars
  theme_minimal() +
  labs(title = "Mean RT as a Function of first_dependent_distance, Split by first_dependent_distancedeprels",
       x = "first_dependent_distance",
       y = "Mean RT",
       color = "first_dependent_distancedeprels") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels if necessary

ggsave(paste("results/first_dependent_distance_and_RT_", args[1], ".pdf", sep=""), width = 20, height = 10)
...
ggplot(rt_means, aes(x = first_dependent_distance, y = Mean_RT)) +
  geom_point(size = 2, color = "blue", alpha = 0.6) +  # Scatter plot
  geom_line(color = "blue", alpha = 0.8) +  # Line plot to connect points
  geom_errorbar(aes(ymin = Mean_RT - SD_RT, ymax = Mean_RT + SD_RT), width = 0.2, alpha = 0.5) +  # Error bars
  theme_minimal() +
  labs(title = "Mean RT as a Function of Raw first_dependent_distance",
       x = "first_dependent_distance",
       y = "Mean RT") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels if necessary
ggsave(paste("results/first_dependent_distance_and_RT_raw_", args[1], ".pdf", sep=""), width = 20, height = 10)
...
# Compute mean RT for each (head_distance, deprel) pair
rt_means <- data %>%
  group_by(head_distance, deprel) %>%
  summarise(Mean_RT = mean(RT, na.rm = TRUE), SD_RT = sd(RT, na.rm = TRUE), .groups = "drop")

# Print results
print("Mean RT aggregated per raw head_distance value and deprel")
print(rt_means)

# Plot results: separate line for each deprel
ggplot(rt_means, aes(x = head_distance, y = Mean_RT, color = as.factor(deprel), group = deprel)) +
  geom_point(size = 2, alpha = 0.6) +  # Scatter plot
  geom_line(alpha = 0.8) +  # Line plot
  geom_errorbar(aes(ymin = Mean_RT - SD_RT, ymax = Mean_RT + SD_RT), width = 0.2, alpha = 0.5) +  # Error bars
  theme_minimal() +
  labs(title = "Mean RT as a Function of head_distance, Split by deprel",
       x = "head_distance",
       y = "Mean RT",
       color = "deprel") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels if necessary

ggsave(paste("results/headdistance_and_RT_", args[1], ".pdf", sep=""), width = 20, height = 10)
...
# Plot results
ggplot(rt_means, aes(x = head_distance, y = Mean_RT)) +
  geom_point(size = 2, color = "blue", alpha = 0.6) +  # Scatter plot
  geom_line(color = "blue", alpha = 0.8) +  # Line plot to connect points
  geom_errorbar(aes(ymin = Mean_RT - SD_RT, ymax = Mean_RT + SD_RT), width = 0.2, alpha = 0.5) +  # Error bars
  theme_minimal() +
  labs(title = "Mean RT as a Function of Raw first_dependent_distance",
       ...
       y = "Mean RT") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels if necessary
ggsave(paste("results/headdistance_and_RT_raw_", args[1], ".pdf", sep=""), width = 20, height = 10)
...
#                              data = data, FUN = mean, na.rm = TRUE),
#               by="head_distance")
#   pr <- merge(pr, aggregate(RT ~ head_distance,
#                             data = data, FUN = mean, na.rm = TRUE),
#               by="head_distance")
#   pr$diff0 <- pr$prs0 - pr$RT
#   pr$diff1 <- pr$prs1 - pr$RT
# 
#   assign(paste("pr", x, sep="."), pr)
# }
# 
...
#   pr <- merge(pr, aggregate(RT ~ first_dependent_distance,
#                             data = data, FUN = mean, na.rm = TRUE),
#               by="first_dependent_distance")
#   pr$diff0 <- pr$prs0 - pr$RT
#   pr$diff1 <- pr$prs1 - pr$RT
...
#   pr <- merge(pr, aggregate(RT ~ left_dependents_distance_sum,
#                             data = data, FUN = mean, na.rm = TRUE),
#               by="left_dependents_distance_sum")
#   pr$diff0 <- pr$prs0 - pr$RT
#   pr$diff1 <- pr$prs1 - pr$RT
...
#   pr <- merge(pr, aggregate(RT ~ left_dependents_count,
#                             data = data, FUN = mean, na.rm = TRUE),
#               by="left_dependents_count")
#   pr$diff0 <- pr$prs0 - pr$RT
#   pr$diff1 <- pr$prs1 - pr$RT
...
#                             data = data, FUN = mean, na.rm = TRUE),
#               by="deprel")
#   pr <- merge(pr, aggregate(RT ~ deprel,
#                             data = data, FUN = mean, na.rm = TRUE),
#               by="deprel")
#   pr$diff0 <- pr$prs0 - pr$RT
#   pr$diff1 <- pr$prs1 - pr$RT
