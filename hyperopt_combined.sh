...
general_hyperopt_params='--min_len_train 3 --min_len_eval_test 3 --shift 2 --global_distr 0 --optimise loglik --masked 0 --combined True --losses {lm:none,} --first_k None --first_k_eval_test None --batch_size 38 --gradient_acc 20 --epochs 100 --early_stop_after 4 --n_trials=100 --eval_interval 1000 --n_warmup_steps 2 --use_steps 1 --max_steps none'
hyperopt_selection='--use_lstm 0 --psyling_dataset geco_train,zuco2_1_train,zuco1_1_train,zuco1_2_train,meco1_train,meco2_train,frank_ET_train --sampler tpe --pruner hyperband --length_weighted 1 --lme_formula GPT~length+position+length.1+length.2+frequency+frequency.1+frequency.2+surprisal+surprisal.1+surprisal.2+(position+length+length.1+length.2+frequency+frequency.1+frequency.2+surprisal+surprisal.1+surprisal.2|WorkerId)+(1|Corpus) --layer_design (0,) --width 8 --depth 8 --n_embd 400 --dropout_attn 0.0 --dropout_resid 0.0 --dropout_ff 0.0 --dropout_embd 0.0 --dropout_lstm 0.0 --learning_rate 1e-3 --d_ff_factor 4 --bias 0 --use_dual_fixed 0'
